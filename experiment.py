...
class SchedulingExperiment:
    ...
    def run(self, report: bool = False) -> Dict[str, Union[int, float]]:
        ...
        if isinstance(self.scheduler, RandomScheduler):
            result = self.scheduler.schedule(self.parcels, self.fleet.trucks)
            if len(result) > 0:
                for parcel in result:
                    self._unscheduled.append(parcel)
        else:
            result = self.scheduler.schedule(self.parcels, self.fleet.trucks)
            if len(result) > 0:
                for parcel in result:
...
def read_parcels(parcel_file: str) -> List[Parcel]:
    ...
    with open(parcel_file, 'r') as file:
        for line in file:
            ...
            temp_parcel = Parcel(pid, volume, source, destination)
            result.append(temp_parcel)
...
def read_distance_map(distance_map_file: str) -> DistanceMap:
    ...
    with open(distance_map_file, 'r') as file:
        for line in file:
            ...
            result.add_distance(c1, c2, distance1, distance2)
...
def read_trucks(truck_file: str, depot_location: str) -> Fleet:
    ...
    with open(truck_file, 'r') as file:
        for line in file:
            ...
            temp_truck = Truck(tid, capacity, depot_location)
            result.add_truck(temp_truck)
