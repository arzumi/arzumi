...
class PriorityQueue(Container):
    ...
    def add(self, item: Any) -> None:
        ...
        if not self._queue:
            ...
            # And find the index to insert the item in.
            # If the item has the greatest priority, i.e more priority
            # Than the rightmost element in the list, we make the index
            # There and we break the loop. Else, We find the position
            # where if we insert The item, the element on the right side
            # will have a greater priority.

            count = len(self._queue) - 1
            index = len(self._queue) - 1

            while count >= 0:
                if self._higher_priority(item, self._queue[count]):
                    index = count + 1
                    break

                if not self._higher_priority(item, self._queue[count]):
                    index = count
